...
import { goBack, resetRoot, navigate } from "../../navigation/utilities"
import { fakeReactotron } from "./reactotronFake"

type Tron = Omit<typeof Reactotron, "log"> & { log: (...args: any[]) => void }
...
export function setupReactotron(customConfig: ReactotronConfig = {}) {
  ...
  if (__DEV__) {
    ...
    // hookup middleware
    if (Platform.OS !== "web") {
      if (config.useAsyncStorage && Reactotron.setAsyncStorageHandler) {
